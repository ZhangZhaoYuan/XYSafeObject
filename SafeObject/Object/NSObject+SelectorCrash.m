...
@interface XYUnrecognizedSelectorSolveObject ()
@end

@implementation XYUnrecognizedSelectorSolveObject
...
id addMethod(id self, SEL _cmd) {
    NSLog(@"XYCrashProtector: unrecognized selector: %@", NSStringFromSelector(_cmd));
    return 0;
}
...
+ (void)xy_enableSelectorProtector {
    ...
    dispatch_once(&onceToken, ^{
        ...
        [object xy_instanceSwizzleMethod:@selector(forwardingTargetForSelector:) replaceMethod:@selector(xy_forwardingTargetForSelector:)];
    });
}

- (id)xy_forwardingTargetForSelector:(SEL)aSelector {
    ...
    XYUnrecognizedSelectorSolveObject *solveObject = [XYUnrecognizedSelectorSolveObject new];
