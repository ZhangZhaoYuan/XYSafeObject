...
@implementation KVOProxy{
    ...
    NSMapTable<id, NSMutableSet<XYCPKVOInfo *> *> *_objectInfosMap; ///< map来维护KVO整个关系
}
...
- (BOOL)xy_addObserver:(id)object KVOinfo:(XYCPKVOInfo *)KVOinfo
{
    [self lock];
    
    // XYCPKVOInfo 存入KVO的信息，object为注册者对象
...
- (void)xy_removeObserver:(id)object keyPath:(NSString *)keyPath block:(void (^)(void))block
{
//    if (!object || !keyPath) {
//        return;
//    }
    ...
    if (info != nil) {
        ...
        [XYSafeLog printCrashMsg:[NSString stringWithFormat:@"Cannot remove an observer %@ for the key path '%@' from %@ because it is not registered as an observer.",object,keyPath,self]];
    }
    [self unlock];
}
...
- (void)xy_removeAllObserver
{
    if (_objectInfosMap) {
        ...
        for (id object in objectInfoMaps) {
            ...
            [infos enumerateObjectsUsingBlock:^(id  _Nonnull obj, BOOL * _Nonnull stop) {
                XYCPKVOInfo *info = (XYCPKVOInfo *)obj;
...
#pragma mark - XYCPKVOInfo
@implementation XYCPKVOInfo {
    @public
    NSString *_keyPath;
    NSKeyValueObservingOptions _options;
    SEL _action;
    void *_context;
    XYCPKVONotificationBlock _block;
}
...
- (instancetype)initWithKeyPath:(NSString *)keyPath
                        options:(NSKeyValueObservingOptions)options
                          block:(nullable XYCPKVONotificationBlock)block
...
static int const XYNSObjectKVOCrashKey;
...
+ (void)xy_enableKVOProtector {
    ...
    dispatch_once(&onceToken, ^{
        ...
        [objc xy_instanceSwizzleMethod:@selector(addObserver:forKeyPath:options:context:) replaceMethod:@selector(xy_addObserver:forKeyPath:options:context:)];
        [objc xy_instanceSwizzleMethod:@selector(removeObserver:forKeyPath:) replaceMethod:@selector(xy_removeObserver:forKeyPath:)];
    });
}

/// 添加观察者，实际添加XYCPKVOInfo -> KVO的管理者，来管理KVO的注册
/**
...
//    XYCPKVOInfo * kvoInfo = [[XYCPKVOInfo alloc] initWithKeyPath:keyPath options:options context:context];
//    __weak typeof(self) wkself = self;
//
//    if ([self.KVOProxy xy_addObserver:wkself KVOinfo:kvoInfo]) {
//        [self xy_addObserver:self.KVOProxy forKeyPath:keyPath options:options context:context];
//    }else {
//        NSLog(@"KVO is more");
//    }
//    [self xy_addObserver:observer forKeyPath:keyPath options:options context:context];
    ...
    @finally {
        ...
        if (observers && [observers containsObject:observer]) {
            //        [XYCrashLog printCrashMsg:[NSString stringWithFormat:@"CrashProtector: Repeat adding the same keyPath observer: %@, keyPath: %@", observer, keyPath]];
            return;
        }
        if (!observers) {
...
//    [self.KVOProxy xy_removeObserver:observer keyPath:keyPath block:^{
//        [self xy_removeObserver:observer forKeyPath:keyPath];
//    }];
    ...
    @finally {
        ...
        if (!observers) {
//            [XYCrashLog printCrashMsg:[NSString stringWithFormat:@"Cannot remove an observer %@ for the key path '%@' from %@ because it is not registered as an observer.",observer,keyPath,self]];
            return;
        }
        // 重复删除观察者
...
- (NSMutableDictionary *)keyPathInfos {
    NSMutableDictionary *dict = objc_getAssociatedObject(self, &XYNSObjectKVOCrashKey);
    if (!dict) {
        dict = [NSMutableDictionary new];
        objc_setAssociatedObject(self, &XYNSObjectKVOCrashKey, dict, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    return dict;
}

- (void)setKeyPathInfos:(NSMutableDictionary *)keyPathInfos {
    objc_setAssociatedObject(self, &XYNSObjectKVOCrashKey, keyPathInfos, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
